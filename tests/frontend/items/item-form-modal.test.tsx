...
const createItemMutateAsync = vi.fn().mockResolvedValue(undefined)
const updateItemMutateAsync = vi.fn().mockResolvedValue(undefined)

vi.mock('@/features/items/api/items-queries', () => ({
...
vi.mock('@/features/items/components/shared/entity-list-chooser', () => ({
...
describe('ItemFormModal — charges behavior', () => {
    ...
    it('submits with charges undefined when creating without touching the selector', async () => {
        render(<ItemFormModal item={null} isOpen={true} onClose={vi.fn()} onSuccess={vi.fn()} />)
    ...
    it('preserves existing charges when editing', async () => {
        ...
        render(<ItemFormModal item={item} isOpen={true} onClose={vi.fn()} onSuccess={vi.fn()} />)
