...
A rota `/sheets` é protegida por guard server-side usando `getCurrentUserFromDb`, evitando redirects client-side prematuros enquanto `/api/auth/session` ainda está carregando. A listagem administrativa usa um único componente responsivo de cards com `useInfiniteAdminSheets` em todos os tamanhos de tela, sem bifurcar por `useIsMobile`; os cards reutilizam `GlassSheetCard` em variante admin, sem ação de exclusão, e o carregamento incremental usa `InfiniteScrollSentinel`.
