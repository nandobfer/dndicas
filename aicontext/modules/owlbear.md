...
Como actions Owlbear rodam em iframe cross-origin, cookies Auth.js podem ser bloqueados como third-party cookies. O site Dndicas em contexto top-level sincroniza um token de bridge limitado em `localStorage` (`OWLBEAR_AUTH_BRIDGE_STORAGE_KEY`) via `/api/owlbear/auth/bridge-token`. A action tenta ler esse token e enviá-lo para `/api/owlbear/session`; quando válido, o backend cria uma `OwlbearSession` autenticada para a sala sem depender do cookie dentro do iframe. O bridge token não substitui a sessão Auth.js principal e deve servir apenas para trocar por uma sessão Owlbear bearer.
...
Para jogadores sem login real no Dndicas, `OwlbearPlayerSheetTab` renderiza `OwlbearSignInPrompt` com link externo para `/sign-in`. Depois do login em uma aba normal e reabertura da action, o bridge token permite criar a sessão Owlbear autenticada e o fluxo volta para a seleção/criação de fichas com `MySheetsContent`, mantendo o vínculo da ficha na metadata da sala.
