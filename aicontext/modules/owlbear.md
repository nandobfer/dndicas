...
Durante a transição pós-login, `401` com usuário logado, erros de rede e respostas `5xx` em `/api/owlbear/session` seguem sendo tratados como transitórios com backoff curto. Recursos pessoais do Dndicas ainda dependem de Clerk: GM anônimo não deve disparar chamadas repetidas para APIs de NPCs pessoais; a UI deve mostrar mensagem de login quando a feature exige conta real.
...
O runtime Owlbear pré-carrega `@owlbear-rodeo/sdk` de forma síncrona em `src/features/owlbear/owlbear-sdk-client.ts`, um módulo client-only consumido por `useOwlbearRuntime`, antes dos effects React. O carregamento deve ficar protegido por `typeof window` para não avaliar o SDK durante o prerender do Next, mas não deve ser movido para `useEffect` nem para `import()` assíncrono: o SDK registra o listener de `message` durante a avaliação do módulo e pode perder o evento `OBR_READY` se for importado tarde demais.
