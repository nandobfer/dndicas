...
- `src/app/api/entity-understanding/chat/route.ts`: rota SSE autenticada para conversar com a IA.
...
6. A sessão local termina ao fechar a janela ou expirar por inatividade.
...
- O chat mantém contexto apenas em estado client-side; a API é stateless.
- O TTL de inatividade local é `ENTITY_UNDERSTANDING_IDLE_TTL_MS = 5 * 60 * 1000`.
