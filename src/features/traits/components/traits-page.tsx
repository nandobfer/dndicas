...
import type { Trait, CreateTraitInput, UpdateTraitInput, TraitsFilters as TraitsFiltersType } from '../types/traits.types';
...
export function TraitsPage() {
    ...
    const [status, setStatus] = React.useState<TraitsFiltersType['status']>('all');
    
    // Debounced search
    const debouncedSearch = useDebounce(search, 500);

    // Filters object for query
    const filters: TraitsFiltersType = React.useMemo(() => ({
        page,
        limit: 10,
    ...
    const handleStatusChange = (value: TraitsFiltersType['status']) => {
        setStatus(value);
        setPage(1);
    };
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p className="text-sm text-white/60 mt-1">
                        Gerencie habilidades e traits do sistema (D&D 5e)
                    </p>
                </div>
