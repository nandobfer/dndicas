...
export interface DeleteTraitDialogProps {
    isOpen: boolean;
    onClose: () => void;
...
export function DeleteTraitDialog({ isOpen, onClose, onConfirm, trait, isDeleting = false }: DeleteTraitDialogProps) {
    ...
    return (
        <GlassModal open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <GlassModalContent size="md">
                <GlassModalHeader>
                    <GlassModalTitle>Excluir Habilidade</GlassModalTitle>
                    <GlassModalDescription>
                        Esta ação removerá a habilidade do catálogo permanentemente. Referências existentes em outros lugares podem ser afetadas.
                    </GlassModalDescription>
                </GlassModalHeader>
                ...
                <div className="mt-6 space-y-6">
                    <div className="space-y-4">
                        ...
                        <div className="flex items-start gap-3 p-3 rounded-lg bg-rose-500/10 border border-rose-500/20">
                            ...
                            <div className="text-sm text-white/80">
                                ...
                                <p>Esta ação é irreversível. A habilidade será removida definitivamente do banco de dados.</p>
                            </div>
                        </div>

                        {/* Trait info */}
                        <div className="p-4 rounded-lg bg-white/5 border border-white/10 space-y-1">
                            <p className="text-xs text-white/40 uppercase tracking-wider mb-2">Habilidade a ser excluída</p>
                            <p className="text-base font-semibold text-white">{trait.name}</p>
                            <p className="text-sm text-white/60">{trait.source}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-white/10">
                        ...
                        >
                            ...
                            {isDeleting ? "Excluindo..." : "Excluir Habilidade"}
                        </button>
                    </div>
                </div>
            </GlassModalContent>
