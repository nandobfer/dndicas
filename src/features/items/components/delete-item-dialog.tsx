...
export function DeleteItemDialog({ isOpen, onClose, onConfirm, item, isDeleting = false }: DeleteItemDialogProps) {
    if (!item) return null
    ...
    return (
        <GlassModal open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <GlassModalContent size="md">
                <GlassModalHeader>
                    <GlassModalTitle>Excluir Item</GlassModalTitle>
                    <GlassModalDescription>
                        Esta ação removerá o item do catálogo permanentemente. Referências existentes em outros lugares podem ser afetadas.
                    </GlassModalDescription>
                </GlassModalHeader>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-start gap-3 p-3 rounded-lg bg-rose-500/10 border border-rose-500/20">
                            ...
                            <div className="text-sm text-white/80">
                                ...
                                <p>Esta ação é irreversível. O item será removido definitivamente do banco de dados.</p>
                            </div>
                        </div>

                        {/* Item info */}
                        <div className="p-4 rounded-lg bg-white/5 border border-white/10 space-y-1">
                            <p className="text-xs text-white/40 uppercase tracking-wider mb-2">Item a ser excluído</p>
                            <p className="text-base font-semibold text-white">{item.name}</p>
                            <p className="text-sm text-white/60">{item.source}</p>
                        </div>
                    </div>
