                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="text-[10px] uppercase font-bold text-white/30 tracking-widest">
                            {type === "armadura" ? (item.acType === "bonus" ? "CA Bônus" : "CA Base") : "Bônus de CA"}
                        </span>
                        <div className="px-2.5 py-0.5 rounded-md text-sm font-bold border border-white/10 bg-white/5 text-white/70">
                            {type === "armadura" ? (item.ac ?? item.acBonus) : `+${item.acBonus ?? item.ac}`}
                        </div>
                    </div>
                    ...
                    {item.armorType && (
                        <div className="flex items-center gap-2">
                            ...
                            {(() => {
                                ...
                                return <div className={cn("px-2.5 py-0.5 rounded-md text-xs font-bold border transition-colors", colors.bg, colors.text, colors.border)}>{config.label}</div>
                            })()}
                        </div>
                    )}
