...
export interface NpcFormModalProps {
    npc: Monster | null
    isOpen: boolean
    onClose: () => void
    onSave: (data: CreateMonsterSchema, npcId?: string) => Promise<void>
...
export function NpcFormModal({
    ...
    React.useEffect(() => {
        ...
        reset({
            ...
            source: npc?.source ?? sourceDefault,
            description: npc?.description ?? "",
            image: npc?.image ?? "",
            status: npc?.status ?? "active",
            type: npc?.type ?? "beast",
            size: npc?.size ?? "M",
            alignment: npc?.alignment ?? "unaligned",
            armorClass: npc?.armorClass ?? 10,
            initiative: npc?.initiative ?? undefined,
            hitPointsFormula: npc?.hitPointsFormula ?? "1d8",
            speed: npc?.speed ?? "9m",
            flySpeed: npc?.flySpeed,
            swimSpeed: npc?.swimSpeed,
            climbSpeed: npc?.climbSpeed,
            attributes: npc?.attributes ?? DEFAULT_ATTRIBUTES,
            savingThrows: npc?.savingThrows ?? {},
            skills: npc?.skills ?? {},
            senses: npc?.senses ?? {},
            sensesAndLanguages: npc?.sensesAndLanguages ?? [],
            challengeRating: npc?.challengeRating ?? "",
            experienceOverride: npc?.experienceOverride,
            proficiencyBonusOverride: npc?.proficiencyBonusOverride,
            languages: npc?.languages ?? "—",
            damageVulnerabilities: npc?.damageVulnerabilities ?? [],
            damageResistances: npc?.damageResistances ?? [],
            damageImmunities: npc?.damageImmunities ?? [],
            conditionImmunities: npc?.conditionImmunities ?? [],
            conditionImmunityNotes: npc?.conditionImmunityNotes ?? "",
            traits: npc?.traits ?? [],
            actions: npc?.actions ?? [],
            bonusActions: npc?.bonusActions ?? [],
            reactions: npc?.reactions ?? [],
            legendaryActions: npc?.legendaryActions ?? [],
            legendaryActionUses: npc?.legendaryActionUses ?? 3,
            lairActions: npc?.lairActions ?? [],
            lairActionInitiative: npc?.lairActionInitiative ?? 20,
            regionalEffects: npc?.regionalEffects ?? [],
        })
    }, [isOpen, npc, reset, sourceDefault])
    ...
    const onSubmit = async (data: CreateMonsterSchema) => {
        ...
        try {
            await onSave(cleaned as CreateMonsterSchema, isEditMode && npc ? npc._id : undefined)
            onClose()
        } catch (error) {
            console.error("[NpcFormModal] Error:", error)
            toast.error(error instanceof Error ? error.message : `Erro ao salvar ${entityLabel}`)
        }
    }

    const modalTitle = isEditMode ? (editTitle ? editTitle(npc?.name ?? "") : title) : title
    const modalSubtitle = isEditMode ? (editSubtitle ?? `Atualize as informações do ${entityLabel.toLowerCase()}`) : (subtitle ?? `Crie um novo ${entityLabel.toLowerCase()}`)
        ...
        <>
            <GlassModal open={isOpen} onOpenChange={(open) => !open && handleCloseAttempt()}>
                <GlassModalContent size="xl" className="max-w-full md:max-w-[78vw]">
                    <GlassModalHeader>
                        <GlassModalTitle>{modalTitle}</GlassModalTitle>
                        <GlassModalDescription>{modalSubtitle}</GlassModalDescription>
                    </GlassModalHeader>

                    <form id="npc-form" onSubmit={handleSubmit(onSubmit as any)} className="space-y-6 mt-4">
                        <GlassStatusSwitch entityLabel={`Status do ${entityLabel}`} description={`${entityLabel}s inativos não aparecem nas buscas`} checked={watch("status") === "active"} onCheckedChange={(checked) => setValue("status", checked ? "active" : "inactive")} disabled={isSubmitting} />

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <GlassInput id="name" label={`Nome do ${entityLabel}`} placeholder="Ex: Dragão Vermelho Adulto" icon={<Skull />} required error={errors.name?.message} {...register("name")} disabled={isSubmitting} />
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <ImageAndDescriptionSection
                            ...
                            aiContextLabel={entityLabel}
                            entityId={npc?._id}
                            placeholder={`Descreva o ${entityLabel.toLowerCase()} detalhadamente...`}
                        />
                        ...
                        <FormSection title="Classificação" icon={<Sparkles className="h-4 w-4" />}>
                            <div className="flex flex-col md:flex-row gap-4">
                                <div className="space-y-2 flex-1 min-w-0">
                                    ...
                                    <OptionAutocomplete value={watch("type") as MonsterType} onChange={(value) => setValue("type", value as MonsterType, { shouldDirty: true })} options={MONSTER_TYPE_OPTIONS} placeholder="Selecione o tipo" title="Tipo" mode="single" className="w-full min-w-0" />
                                </div>
                                <div className="space-y-2 flex-1 min-w-0">
