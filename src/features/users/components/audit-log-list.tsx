...
/**
 * Responsive Audit Log List for mobile view.
 * Features infinite scroll and specialized AuditLogCard renderer.
 */
export function AuditLogList({ 
    ...
    if (isLoading && items.length === 0) {
        return (
            <div className="py-12 flex justify-center">
                <LoadingState variant="spinner" message={`Carregando logs...`} />
            </div>
        )
    }
    ...
    if (!isLoading && items.length === 0) {
        return (
            <div className="py-12">
                <EmptyState title={`Nenhum log encontrado`} description="Tente ajustar os filtros." icon={Clock} />
            </div>
        )
    }
                    ...
                    >
                        <GlassCard className="relative overflow-hidden">
                            <GlassCardContent className="p-4 pt-6">
                                <AuditLogCard log={item} onClick={() => onLogClick(item)} />
                            </GlassCardContent>
                        </GlassCard>
                    </motion.div>
                ))}
