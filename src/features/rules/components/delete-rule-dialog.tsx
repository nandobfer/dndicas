...
import type { Reference } from "../types/rules.types"
...
export function DeleteRuleDialog({ isOpen, onClose, onConfirm, rule, isDeleting = false }: DeleteRuleDialogProps) {
    if (!rule) return null
    ...
    return (
        <GlassModal open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <GlassModalContent size="md">
                <GlassModalHeader>
                    <GlassModalTitle>Excluir Regra</GlassModalTitle>
                    <GlassModalDescription>
                        Esta ação removerá a regra do catálogo permanentemente. Referências existentes em outros lugares podem ser afetadas.
                    </GlassModalDescription>
                </GlassModalHeader>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-start gap-3 p-3 rounded-lg bg-rose-500/10 border border-rose-500/20">
                            ...
                            <div className="text-sm text-white/80">
                                ...
                                <p>Esta ação é irreversível. A regra será removida definitivamente do banco de dados.</p>
                            </div>
                        </div>

                        {/* Rule info */}
                        <div className="p-4 rounded-lg bg-white/5 border border-white/10 space-y-1">
                            <p className="text-xs text-white/40 uppercase tracking-wider mb-2">Regra a ser excluída</p>
                            <p className="text-base font-semibold text-white">{rule.name}</p>
                            <p className="text-sm text-white/60">{rule.source}</p>
                        </div>
                    </div>
