...
import ImageExtension, { Image } from '@tiptap/extension-image'
  ...
  return (
    <div className="flex flex-wrap gap-1 p-2 border-b border-border/10 bg-black/20 rounded-t-lg items-center">
      ...
      <Button
        ...
        className={cn(editor.isActive('heading', { level: 1 }) ? 'bg-white/10' : '', 'h-8 w-8 p-0')}
      >
      ...
      <Button
        ...
        className={cn(editor.isActive('heading', { level: 2 }) ? 'bg-white/10' : '', 'h-8 w-8 p-0')}
      >
...
export function RichTextEditor({
  ...
  const addImage = useCallback(() => {
    ...
    input.click()
  }, [uploadImage]) // editor dependency handled in render potentially, but cleaner to use state or ref if needed
  ...
  return (
    <div className={cn(
      'rounded-lg border border-white/10 bg-white/5 overflow-hidden focus-within:ring-1 focus-within:ring-primary/50 transition-all group',
