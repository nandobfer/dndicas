...
export default function StorageExamplePage() {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            setError((err as any).response?.data?.error || "Erro ao fazer upload")
        } finally {
            setUploading(false)
        }
    }
    ...
    const handleGetDownloadUrl = async () => {
        ...
        try {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            setError((err as any).response?.data?.error || "Erro ao gerar URL de download")
        } finally {
    ...
    return (
        <div className="flex flex-col gap-4">
            <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">Demonstração de upload e download de arquivos (S3/Minio)</p>
            </div>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        {uploadResult && (
                            <div className="p-3 bg-primary/10 border border-primary/20 rounded-md">
                                ...
                                <p className="text-xs text-muted-foreground mt-1">Key: {uploadResult.key}</p>
                            </div>
                        )}
                    </CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        {uploadResult && !downloadUrl && (
                            <Button onClick={handleGetDownloadUrl} disabled={downloading} className="w-full">
                                {downloading ? (
                                    <>
                                        <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                        Gerando URL...
                        ...
                        {downloadUrl && (
                            <div className="space-y-3">
                                ...
                                <Button onClick={() => window.open(downloadUrl, "_blank")} variant="secondary" className="w-full">
                                    <Download className="mr-2 h-4 w-4" />
                                    Abrir Arquivo
                                </Button>
                            </div>
            ...
            <Card>
                ...
                <CardContent className="space-y-2 text-sm text-muted-foreground">
                    <p>Este exemplo demonstra o uso do serviço de storage centralizado.</p>
                    <p>
                    ...
                    <ul className="list-disc list-inside ml-4">
                        <li>
                            <code className="bg-muted px-1 rounded">POST /api/core/storage/upload</code> - Upload de arquivo
                        </li>
                        <li>
                            <code className="bg-muted px-1 rounded">GET /api/core/storage/download?key=...</code> - URL de download
                        </li>
                    </ul>
                    <p>Os arquivos são armazenados no S3/Minio configurado nas variáveis de ambiente.</p>
                </CardContent>
            </Card>
        </div>
    )
