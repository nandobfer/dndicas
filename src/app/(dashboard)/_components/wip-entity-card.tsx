...
export function WipEntityCard({
    ...
    return (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.4 + index * 0.05 }}>
            <GlassCard className="h-[212px] border-white/5 bg-white/[0.02] hover:bg-white/[0.04] transition-colors relative group overflow-hidden flex flex-col">
                <GlassCardHeader className="pb-0 shrink-0">
                    <div className="flex items-start justify-between">
                        ...
                        <div className="text-[10px] font-bold text-white/20 group-hover:text-white/40 transition-colors uppercase tracking-wider">Vem aí</div>
                    </div>
                ...
                <GlassCardContent className="pt-4 mt-auto shrink-0 pb-6">
                    <div className="space-y-3">
                        ...
                        <div className="flex items-end gap-0.5 h-8 w-full opacity-20 group-hover:opacity-30 transition-opacity">
                            {[...Array(12)].map((_, i) => (
                                <div key={i} className="flex-1 rounded-t-[1px] bg-white/40" style={{ height: `${20 + ((i * 7 + index * 13) % 80)}%` }} />
                            ))}
                        </div>
                    </div>
                </GlassCardContent>
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px] z-20">
                    <div className="flex flex-col items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest text-white/60">Em Breve</span>
                    </div>
                </div>
            </GlassCard>
        </motion.div>
