...
export function RulesEntityCard({ stats, loading, index }: { stats?: { total: number; active: number; growth?: Array<{ count: number }> }; loading: boolean; index: number }) {
    return <EntityCard entityType="Regra" stats={stats} loading={loading} index={index} title="Regras" icon={Scroll} description="Catálogo de regras e diretrizes customizadas" />
}
