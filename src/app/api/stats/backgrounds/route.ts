...
export async function GET(_request: NextRequest) {
    try {
        ...
        const activeCount = await Background.countDocuments({ status: "active" })

        // Growth data
        const firstRecord = await Background.findOne().sort({ createdAt: 1 }).select("createdAt").lean()
        const growth = []
        ...
        console.error("[Backgrounds Stats API] Error:", error)
