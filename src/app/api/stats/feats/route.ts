...
export async function GET(_request: NextRequest) {
    try {
        ...
        const activeCount = await Feat.countDocuments({ status: "active" })

        // Growth data
        const firstRecord = await Feat.findOne().sort({ createdAt: 1 }).select("createdAt").lean()
        const growth = []
        ...
        console.error("[Feats Stats API] Error:", error)
