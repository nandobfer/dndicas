...
import { spacing } from "@/lib/config/theme-config"
  ...
  const itemContent = (
      <motion.div
          className={cn(
              ...
              "pl-[18px]", // Centered icon position in collapsed state (56px inner)
          )}
...
export const SidebarSection: React.FC<SidebarSectionProps> = ({ isExpanded, children }) => {
    // 280px (expanded) - 40px (20px margin each side) = 240px
    const expandedWidth = spacing.sidebar.expanded - 40
    const collapsedWidth = 32

    return (
        <div className="space-y-1">
            <motion.div
                className="mx-auto my-4 h-px bg-white/10"
                initial={false}
                animate={{
