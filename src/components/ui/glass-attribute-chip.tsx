 ...
 *
 * Displays D&D attribute with configured color scheme and abbreviation or full name.
 ...
 *
 * // Full name
 * <GlassAttributeChip attribute="Força" showFull />
 * // Renders: "Força" with amber color
 *
 * // With bonus
 ...
 * ```
 */
export function GlassAttributeChip({ attribute, bonus, size = "sm", showFull = false, className }: GlassAttributeChipProps) {
    ...
    if (!config) {
        ...
        return (
            <Chip size={size} className={cn("bg-white/10 text-white/70 border-white/10", className)}>
                {showFull ? attribute : attribute.slice(0, 3).toUpperCase()}
    ...
    return (
        <Chip size={size} className={cn("text-[10px] py-0.5 px-1.5 h-auto border shrink-0", config.badge, config.border, className)}>
            {showFull ? attribute : config.abbreviation}
            {bonus !== undefined && ` +${bonus}`}
        </Chip>
    )
}
